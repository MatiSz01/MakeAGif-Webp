# =============================================================================
# MakeAGIF v3.1.4 — DND Prototype (NLE trim/playhead + frame-exact preview)
# =============================================================================
# Forked from v3.1.3; macOS App Store–style release with bundled magick + v3.1.1 trim UX.
# v3.1.4 changelog:
...
APP_TITLE = f"MakeAGIF/WEBP v3.1.4 — By {APP_AUTHOR}"
...
class DropZoneWidget(QWidget):
    ...
    def update_trim_previews(self, video_path, in_sec, out_sec, duration_sec, fps=None):
        ...
        pix_in = extract_frame_at(video_path, in_sec, fps=fps, frame_index=in_fi)
        pix_out = extract_frame_at(video_path, 0.0, fps=fps, frame_index=last_fi)
        if pix_in:
...
class MainWindow(QMainWindow):
    ...
    def _update_batch_trim_previews(self, video_path, in_sec, out_sec, dur, fps):
        ...
        out_excl_fi = source_frame_index_from_sec(out_sec, fps) if fps else 0
        last_fi = max(0, out_excl_fi - 1)
        pix_in = extract_frame_at(
            video_path, in_sec, fps=fps, tw=120, th=68, frame_index=in_fi
        )
        pix_out = extract_frame_at(
            video_path, 0.0, fps=fps, tw=120, th=68, frame_index=last_fi
        )
        if pix_in:
