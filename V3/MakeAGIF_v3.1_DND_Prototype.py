# =============================================================================
# MakeAGIF v3.1 — DND Prototype
# =============================================================================
# v3.1 changelog (in progress, growing as features land):
...
#     MakeAGIF_v3.1_win.spec and MakeAGIF_v3.1_mac.spec.
#
...
# --- Constants ---
SCRIPT_VERSION = "v3.1 Beta"
APP_AUTHOR = "Matias Szteinberg"
APP_TITLE = f"MakeAGIF/WEBP v3.1 Beta — By {APP_AUTHOR}"
...
def get_tool_path(name):
    ...
    ./tools/ (or PyInstaller's MEIPASS extracted bundle when frozen),
    ...
    p = os.path.join(base_path, "tools", fname)
    if os.path.exists(p): return p
    # Check portable MEIPASS (PyInstaller --onefile)
    if getattr(sys, 'frozen', False):
        meipass = getattr(sys, '_MEIPASS', None)
        if meipass:
            p = os.path.join(meipass, "tools", fname)
            if os.path.exists(p): return p
    # Fallback to system PATH (bare name; OS handles resolution)
    return fname
...
class TrimDialog(QDialog):
    ...
    def _seek_ratio(self, ratio):
        ...
        QTimer.singleShot(0, self._apply_precise_preview)
    ...
    def _apply_precise_preview(self):
        """When paused, decode the current frame with cv2/ffmpeg and paint it
        over QVideoWidget.  While playing the native surface is shown."""
        if self.player.playbackState() == QMediaPlayer.PlayingState:
            self._precise_frame.hide()
            return
        ...
        tw = min(vw, 1280)
        th = min(vh, 720)
        pix = extract_frame_at(self.task.path, sec, fps=self.fps, tw=tw, th=th)
