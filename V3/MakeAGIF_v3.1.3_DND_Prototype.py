# =============================================================================
# MakeAGIF v3.1.3 — DND Prototype (NLE trim/playhead + frame-exact preview)
# =============================================================================
...
APP_TITLE = f"MakeAGIF/WEBP v3.1.3 — By {APP_AUTHOR}"
...
def source_frame_index_from_sec(sec, fps, max_idx=None):
    """0-based source frame index for *sec* (matches NLE TC round-trip)."""
    if not fps or float(fps) <= 0:
        return 0
    fi = int(round(float(sec) * float(fps)))
...
def last_included_frame_index_from_out(out_sec, fps, max_idx=None):
    ...
    out_excl = source_frame_index_from_sec(out_sec, fps, max_idx)
    return max(0, out_excl - 1)
...
class SceneDetectorWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            if self.fps > 0:
                cuts = sorted(
                    sec_from_source_frame_index(fi, self.fps)
                    for fi in cut_frame_indices
                )
            else:
                cuts = sorted(cut_frame_indices)
...
class TrimDialog(QDialog):
    ...
    def __init__(self, task, parent=None, initial_in_sec=0.0, initial_out_sec=None):
        ...
        self._precise_coalesce = None  # (gen, path, frame_idx, fps, tw, th, vw, vh)
    ...
    def _frame_index_from_sec(self, sec):
        """Integer source frame index for a timestamp (clamped)."""
        if not self.fps or self.fps <= 0:
            return None
        return source_frame_index_from_sec(sec, self.fps, self._max_frame_idx)
    ...
    def _player_frame_index(self):
        ...
        if self._step_frame_idx is not None:
            return self._step_frame_idx
        return self._frame_index_from_sec(self.player.position() / 1000.0)

    def _park_at_frame_index(self, fi):
    ...
    def _frame_align_cuts_sec(self, cuts):
        """Snap detected scene PTSs to source-frame boundaries for markers/snap."""
        if not cuts:
            return []
        dur = self._media_duration_sec()
        if not self.fps or self.fps <= 0:
            return sorted({float(c) for c in cuts if 0.0 < float(c) < dur})
        max_fi = self._effective_max_frame_idx_for_cuts()
        aligned = []
        seen_fi = set()
        for c in cuts:
            fi = source_frame_index_from_sec(c, self.fps, max_fi)
            if fi <= 0 or fi in seen_fi:
                continue
            seen_fi.add(fi)
        ...
        # Safety: under-reported t_frames used to clamp every cut to frame 0 → dropped.
        fallback = []
        for c in cuts:
            q = self._quantize_to_source_frame(float(c))
            if 0.0 < q < dur:
                fallback.append(q)
    ...
    def _seek_ratio(self, ratio, *, immediate_precise=False):
        ...
        if self.fps > 0:
            fi = self._frame_index_from_sec(ratio * self.duration_sec)
            self._step_frame_idx = fi
            sec = self._sec_from_frame_index(fi)
            self._refresh_tc(sec)
    ...
    def _flush_precise_preview_async(self):
        ...
        with self._precise_lock:
            if self._precise_worker_busy:
                self._precise_coalesce = (gen, path, fi, fps, tw, th, vw, vh)
                return
            self._precise_worker_busy = True
        threading.Thread(
            target=self._precise_thread_job,
            args=(gen, path, fi, fps, tw, th, vw, vh),
            daemon=True,
        ).start()

    def _precise_thread_job(self, gen, path, frame_idx, fps, tw, th, vw, vh):
        png = None
        try:
    ...
    def _on_precise_decode_finished(self, png_bytes, gen, vw, vh):
        ...
        if next_job is not None and not self._precise_shutdown:
            gen2, path2, fi2, fps2, tw2, th2, vw2, vh2 = next_job
            threading.Thread(
                target=self._precise_thread_job,
                args=(gen2, path2, fi2, fps2, tw2, th2, vw2, vh2),
                daemon=True,
            ).start()
    ...
    def _update_trim_thumb(self, which):
        ...
        *which* must be ``"in"`` or ``"out"``.  Uses frame-index seeking
        ...
        if which == "in":
            ...
            out_excl = source_frame_index_from_sec(self.out_sec, self.fps)
            fi = max(0, out_excl - 1)
            lbl = self._trim_thumb_out
        pix = extract_frame_at(
            self.task.path, 0.0, fps=self.fps, tw=106, th=60, frame_index=fi
        )
        if pix:
            lbl.setPixmap(pix)
        else:
    ...
    def set_in(self):
        ...
        if fi is None:
                ...
                self._displayed_frame_sec(), self.fps, self._max_frame_idx
            )
        if self._step_frame_idx is None and self.cuts_sec:
            fi = self._snap_frame_index(fi)
        fi = max(0, min(fi, self._max_frame_idx))
    ...
    def set_out(self):
        ...
        if source_frame_index_from_sec(self.out_sec, self.fps) <= in_fi:
            self.out_sec = exclusive_out_sec_from_included_frame(
                min(in_fi, self._max_frame_idx), self.fps, cap
            )
        self.timeline.set_out(self.out_sec / self.duration_sec)
        self.lbl_out.setText(f"OUT: {self._sec_to_tc(self.out_sec)}")
        self._update_trim_thumb("out")
        self._park_at_frame_index(
            last_included_frame_index_from_out(self.out_sec, self.fps, self._max_frame_idx)
        )
    ...
    def goto_out(self):
        ...
        self._park_at_sec(self.out_sec)
    ...
    def _on_segment_selected(self, start_ratio, end_ratio):
        ...
        dur = self._media_duration_sec()
        in_fi = source_frame_index_from_sec(start_ratio * dur, self.fps, self._max_frame_idx)
        out_excl_fi = source_frame_index_from_sec(end_ratio * dur, self.fps)
    ...
    def _snap_frame_index(self, fi):
        ...
        if not self.chk_snap.isChecked() or not self.cuts_sec or not self.fps:
            return fi
        cut_fis = [
            source_frame_index_from_sec(c, self.fps) for c in self.cuts_sec
        ]
        nearest_fi = min(cut_fis, key=lambda cf: abs(cf - fi))
    ...
    def _on_scene_done(self, cuts):
        dur = self._media_duration_sec()
        raw_n = len(cuts)
        self.cuts_sec = self._frame_align_cuts_sec(
            [c for c in cuts if 0.0 <= c <= dur + 0.05]
        )
        ratios = [c / dur for c in self.cuts_sec] if dur > 0 else []
    ...
    def _try_warm_start_scenes(self):
        ...
        dur = self._media_duration_sec()
        self.cuts_sec = self._frame_align_cuts_sec(
            [c for c in cached if 0.0 <= c <= dur + 0.05]
        )
        ratios = [c / dur for c in self.cuts_sec] if dur > 0 else []
        self.timeline.set_cuts(ratios)
        self.lbl_scene.setText(
    ...
    def _finalize_trim_frame_alignment(self):
        ...
        if not self.fps or self.fps <= 0:
            return
        in_i = source_frame_index_from_sec(self.in_sec, self.fps, self._max_frame_idx)
        out_excl_i = source_frame_index_from_sec(self.out_sec, self.fps)
...
def _extract_frame_png_bytes(video_path, sec, fps, tw, th, frame_index=None):
    ...
    Uses the same ``-accurate_seek`` + ``-ss`` path as trim/export so the
  preview matches encoded output. Pass ``frame_index`` when known (preferred).
    """
    try:
        tw = max(32, int(tw))
        th = max(32, int(th))
        ...
        try:
            ...
            if cap.isOpened():
                if fps and float(fps) > 0:
                        ...
                        int(frame_index)
                        if frame_index is not None
                        else source_frame_index_from_sec(sec, fps)
                    )
