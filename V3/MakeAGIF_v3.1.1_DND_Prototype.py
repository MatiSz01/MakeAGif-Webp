# =============================================================================
# MakeAGIF v3.1.1 — DND Prototype (trim preview: async decode + debounced scrub)
# =============================================================================
# Forked from v3.1; same feature set with trim-dialog responsiveness tweaks.
...
#     MakeAGIF_v3.1.1_win.spec (Windows) / MakeAGIF_v3.1.1_mac.spec (macOS arm64).
#     See MACOS_BUILD.md for Apple Silicon build steps (must run on a Mac).
#
# v3.1.1 patch:
...
APP_TITLE = f"MakeAGIF/WEBP v3.1.1 — By {APP_AUTHOR}"
...
class ConversionEngine:
    ...
    def run_iterative_search(self, ui_vals, w, h, common, specs, final_out_path):
        ...
        if not best_res and closest_any['file_path'] and not p2_skip and target_fps > min_fps:
            ...
            # Build a descending list of FPS tiers to try between target_fps
            # and min_fps. E.g. target=22, min=8 → step ~4 → [18, 14, 10, 8].
            fps_step = max(2, (target_fps - min_fps) // 4)
            fps_tiers = list(range(target_fps - fps_step, min_fps - 1, -fps_step))
            if fps_tiers and fps_tiers[-1] != min_fps:
                fps_tiers.append(min_fps)
            if not fps_tiers:
                fps_tiers = [min_fps]
            max_p2_total = 12
            p2_used = 0
        ...
        try:
            ...
            for line in iter(self._proc.stderr.readline, ''):
                ...
                if m_t and self.duration_sec > 0:
                    ...
                    if pct != last_pct:
                        last_pct = pct
                        self.progress.emit(pct, len(cuts))
            try: self._proc.wait(timeout=2)
            except Exception: pass
            if self._cancel:
                return
            self.progress.emit(100, len(cuts))
...
class _PrecisePreviewBridge(QObject):
    ...
    decode_done = Signal(object, int, int, int)  # png_bytes|None, gen, vw, vh
...
class TrimDialog(QDialog):
    ...
    def __init__(self, task, parent=None, initial_in_sec=0.0, initial_out_sec=None):
        ...
        self._precise_coalesce = None  # (gen, path, sec, fps, tw, th, vw, vh)
        self._precise_shutdown = False
        self._precise_seek_timer = QTimer(self)
        self._precise_seek_timer.setSingleShot(True)
        self._precise_seek_timer.setInterval(55)
        self._precise_seek_timer.timeout.connect(self._flush_precise_preview_async)
        self._precise_resize_timer = QTimer(self)
        self._precise_resize_timer.setSingleShot(True)
        self._precise_resize_timer.setInterval(90)
        self._precise_resize_timer.timeout.connect(self._flush_precise_preview_async)
        ...
        if task.specs.get("is_vfr"):
            ...
            detail = ""
            if r_fps and a_fps:
                detail = f" (declared {r_fps:.2f} fps · measured avg {a_fps:.2f} fps)"
            vfr_banner = QLabel(
        ...
        nle_hint = QLabel(
            "Paused: exact frame preview (same decode as GIF/WebP trim).  "
            "Play: smooth Qt preview.  OUT = first excluded frame (NLE standard)."
        )
        ...
        self.btn_loop.setToolTip("Loop playback between IN and OUT points (preview the final trim)")
        ...
        # Show trim thumbnails if opening with an existing trim range
        has_trim = (self.in_sec > 0.01 or self.out_sec < self.duration_sec - 0.01)
        if has_trim:
        ...
        self.player.play()
        self.player.pause()
        self._seek_ratio(self.in_sec / self.duration_sec, immediate_precise=True)
    ...
    def _seek_ratio(self, ratio, *, immediate_precise=False):
        ...
        if self.fps > 0:
            ...
            self._step_frame_idx = int(round(sec * self.fps))
            self._step_frame_idx = max(0, min(self._step_frame_idx, self._max_frame_idx))
            sec = self._step_frame_idx / self.fps
        ...
        if immediate_precise:
            self._apply_precise_preview()
        else:
            self._schedule_precise_preview_debounced()
    ...
    def _step_sec(self, delta):
        ...
        if self.fps > 0:
            ...
            if self._step_frame_idx is not None:
                cur_frame = self._step_frame_idx
            else:
                cur_frame = int(round(
                    (self.player.position() / 1000.0 + 0.4 / self.fps) * self.fps
                ))
            target_frame = max(0, min(cur_frame + step_frames, self._max_frame_idx))
            self._step_frame_idx = target_frame
            new_ms = int(round(target_frame / self.fps * 1000))
            self._refresh_tc(target_frame / self.fps)
        else:
    ...
    def _on_loop_toggled(self, checked):
        """Loop mode: forces playback between IN and OUT continuously."""
        self._step_frame_idx = None
    ...
    def _jkl_stop(self):
        ...
        self._update_speed_indicator()
        if self.fps > 0:
            self._step_frame_idx = int(round(
                (self.player.position() / 1000.0 + 0.4 / self.fps) * self.fps
            ))
            self._step_frame_idx = max(0, min(self._step_frame_idx, self._max_frame_idx))
            self._refresh_tc(self._step_frame_idx / self.fps)
    ...
    def _k_step_tick(self):
        ...
        self._step_sec(self._k_step_dir / self.fps)
    ...
    def _schedule_precise_preview_debounced(self):
        """Timeline scrub: wait briefly so rapid drags coalesce into one decode."""
        if self.player.playbackState() == QMediaPlayer.PlayingState:
            return
        self._precise_seek_timer.start(55)

    def _apply_precise_preview(self):
        """Queue frame-exact overlay (async). Cancels pending debounced seeks."""
    ...
    def _flush_precise_preview_async(self):
        ...
        with self._precise_lock:
            if self._precise_worker_busy:
                self._precise_coalesce = (gen, path, sec, fps, tw, th, vw, vh)
                return
            self._precise_worker_busy = True
        threading.Thread(
            target=self._precise_thread_job,
            args=(gen, path, sec, fps, tw, th, vw, vh),
            daemon=True,
        ).start()

    def _precise_thread_job(self, gen, path, sec, fps, tw, th, vw, vh):
        png = None
        try:
            png = _extract_frame_png_bytes(path, sec, fps, tw, th)
        except Exception:
            png = None
        self._precise_bridge.decode_done.emit(png, gen, vw, vh)

    @Slot(object, int, int, int)
    def _on_precise_decode_finished(self, png_bytes, gen, vw, vh):
        if self._precise_shutdown:
            with self._precise_lock:
                self._precise_worker_busy = False
        ...
        if next_job is not None and not self._precise_shutdown:
            gen2, path2, sec2, fps2, tw2, th2, vw2, vh2 = next_job
            threading.Thread(
                target=self._precise_thread_job,
                args=(gen2, path2, sec2, fps2, tw2, th2, vw2, vh2),
                daemon=True,
            ).start()
    ...
    def toggle_play(self):
        ...
        if self.player.playbackState() == QMediaPlayer.PlayingState:
            ...
            self._precise_frame.hide()
            self._step_frame_idx = None
            # In loop mode, always start from IN point
            if self._loop_active:
                self.player.setPosition(int(self.in_sec * 1000))
            self._reset_stutter()
            self.player.play()
    ...
    def _displayed_frame_sec(self):
        ...
        When a logical frame index is active (set by frame-stepping),
        it takes priority over the player's imprecise ms position.
        Otherwise we derive from QMediaPlayer.position() with a forward
        bias to compensate for its integer-ms truncation."""
        if self._step_frame_idx is not None and self.fps > 0:
            return self._step_frame_idx / self.fps
        pos_sec = self.player.position() / 1000.0
        if self.fps > 0:
            bias = 0.4 / self.fps
            pos_sec += bias
        return self._quantize_to_source_frame(pos_sec)
    ...
    def _quantize_to_source_frame(self, sec):
        ...
        frame_idx = int(round(sec * self.fps))
        return frame_idx / self.fps

    def _update_trim_thumb(self, which):
        """Extract and display the frame-exact thumbnail for IN or OUT.
        *which* must be ``"in"`` or ``"out"``.  Uses frame-index seeking
        so the thumbnail is pixel-identical to the monitor frame."""
        if which == "in":
            sec = self.in_sec
            lbl = self._trim_thumb_in
        else:
            sec = self.out_sec - (1.0 / self.fps) if self.fps > 0 else self.out_sec
            sec = max(0.0, sec)
            lbl = self._trim_thumb_out
        pix = extract_frame_at(self.task.path, sec, fps=self.fps, tw=106, th=60)
        if pix:
            lbl.setPixmap(pix)
        else:
            lbl.clear(); lbl.setText("—")
        lbl.setVisible(True)
    ...
    def set_in(self):
        displayed = self._displayed_frame_sec()
        # Only snap to cuts when the position comes from an imprecise
        # source (timeline click / playback stop).  When the user has
        # stepped frame-by-frame the position is already exact and
        # snapping would override their intent — matching NLE behaviour.
        if self._step_frame_idx is None:
            displayed = self._snap_sec(displayed)
        displayed = self._quantize_to_source_frame(displayed)
        self.in_sec = displayed
    ...
    def set_out(self):
        ...
        if self._step_frame_idx is None:
            displayed = self._snap_sec(displayed)
        displayed = self._quantize_to_source_frame(displayed)
        # OUT is exclusive in ffmpeg: advance by one frame so the frame
        # the user sees on screen IS included as the last output frame.
        displayed += (1.0 / self.fps) if self.fps > 0 else 0.0
        cap = self._default_out_sec if self.fps > 0 else self.duration_sec
        displayed = min(displayed, cap)
        if displayed <= self.in_sec:
            displayed = self.in_sec + (1.0 / self.fps)
        self.out_sec = displayed
    ...
    def goto_in(self):
        ...
        Pauses playback first — same as a manual scrub via click — so the
        user lands AT the mark instead of immediately drifting forward."""
        self._goto_sec(self.in_sec)

    def goto_out(self):
        """Jump the playhead to the OUT point (NLE Shift+O).
        Note that OUT is the EXCLUSIVE boundary (the first frame NOT
        included), so visually you'll see the frame AFTER the last included
        frame. That matches Premiere/Resolve, and matches what the engine
        will actually trim away."""
        self._goto_sec(self.out_sec)

    def _goto_sec(self, sec):
        """Common path for both goto_in/out: pause + clamped seek. Routed
        through the player rather than the timeline so we don't have to
        re-derive the ratio (the player will fire positionChanged →
        _refresh_tc → timeline.set_position, which is the canonical update
        chain anyway)."""
        if self.fps > 0:
            self._step_frame_idx = int(round(sec * self.fps))
            self._step_frame_idx = max(0, min(self._step_frame_idx, self._max_frame_idx))
        ...
        self.player.pause()
        self.btn_play.setText("▶ PLAY")
        new_ms = max(0, min(int(float(sec) * 1000),
                            int(self.duration_sec * 1000)))
        self.player.setPosition(new_ms)
        self._apply_precise_preview()
    ...
    def _on_segment_selected(self, start_ratio, end_ratio):
        ...
        if self.fps > 0:
            self._step_frame_idx = int(round(self.in_sec * self.fps))
            self._step_frame_idx = max(0, min(self._step_frame_idx, self._max_frame_idx))
        self.player.pause()
        self.btn_play.setText("▶ PLAY")
        self.player.setPosition(int(self.in_sec * 1000))
    ...
    def _jump_to_cut(self, direction):
        ...
        self._goto_sec(self._quantize_to_source_frame(target))
    ...
    def _finalize_trim_frame_alignment(self):
        ...
        if not self.fps or self.fps <= 0:
            return
        in_i = int(round(self.in_sec * self.fps))
        in_i = max(0, min(in_i, self._max_frame_idx))
        out_excl_i = int(round(self.out_sec * self.fps))
        out_excl_i = max(in_i + 1, min(out_excl_i, self._total_source_frames))
        self.in_sec = in_i / self.fps
        self.out_sec = out_excl_i / self.fps
...
def _extract_frame_png_bytes(video_path, sec, fps, tw, th):
    """Frame-exact grab as PNG bytes (no Qt). Same seek rules as ``extract_frame_at``.

    Intended for background threads — avoids constructing QPixmap off the GUI thread.
    """
    try:
        tw = max(32, int(tw))
        th = max(32, int(th))
    ...
    if HAS_CV2:
        ...
        try:
            ...
            if cap.isOpened():
                if fps and fps > 0:
                    cap.set(cv2.CAP_PROP_POS_FRAMES, max(0, round(sec * fps)))
                else:
    ...
    try:
        cmd = [
            FFMPEG_PATH, "-y", "-ss", f"{max(0, sec):.6f}", "-i", video_path,
            "-vframes", "1", "-vf", f"scale={tw}:-1",
            tmp_png,
        ]
        flags = 0x08000000 if os.name == "nt" else 0
        proc = subprocess.run(
...
def extract_frame_at(video_path, sec, fps=None, tw=142, th=80):
    """Frame-exact single-frame grab.  Returns a scaled QPixmap or None.

    When *fps* is provided the seek uses ``CAP_PROP_POS_FRAMES`` with the
    frame index ``round(sec * fps)`` so the extracted pixel data is
    identical to what QMediaPlayer shows at the same quantized PTS.
    Without *fps* it falls back to millisecond-based seeking.
    """
    if HAS_CV2:
        try:
            cap = cv2.VideoCapture(video_path)
            ...
            pass
    try:
        tmp_png = os.path.join(tempfile.gettempdir(), f"_frame_{os.getpid()}_{sec:.6f}.png")
        cmd = [
            FFMPEG_PATH, "-y", "-ss", f"{max(0, sec):.6f}", "-i", video_path,
            "-vframes", "1", "-vf", f"scale={tw}:-1",
            tmp_png,
...
class DropZoneWidget(QWidget):
    ...
    def update_trim_previews(self, video_path, in_sec, out_sec, duration_sec, fps=None):
        """Extract and display the first/last frames of the trimmed region.
        ...
        pix_in = extract_frame_at(video_path, in_sec, fps=fps)
        last_frame_sec = max(0.0, out_sec - (1.0 / fps if fps and fps > 0 else 0.04))
        pix_out = extract_frame_at(video_path, last_frame_sec, fps=fps)
        if pix_in:
            self._thumb_in.setPixmap(pix_in)
        else:
            self._thumb_in.setText("—")
        if pix_out:
            self._thumb_out.setPixmap(pix_out)
        else:
            self._thumb_out.setText("—")
...
class SettingsPanel(QFrame):
    def __init__(self):
        ...
        self.t_end.setToolTip(
            "HH:MM:SS:FF, HH:MM:SS, or seconds. Empty = end of clip\n"
            "(the placeholder shows the actual end TC of the loaded source)."
        )
    ...
    def set_current_fps(self, fps, duration_sec=None):
        ...
        if duration_sec is not None:
            ...
            if self._current_duration_sec > 0 and self._current_fps > 0:
                tc_end = format_seconds_as_tc_frames(
    ...
    def _refresh_trim_length(self):
        ...
        if out_raw:
            out_sec = parse_trim_to_seconds(out_raw, default=dur, fps=fps)
            if out_sec is None: out_sec = dur
        else:
            out_sec = dur
...
class MainWindow(QMainWindow):
    ...
    def show_about(self):
        ...
        lbl_v = QLabel(f"{SCRIPT_VERSION}  ·  Windows build")
        ...
        #   - gifski (AGPL):     high-quality GIF encoder used for non-alpha GIFs
        #   - ImageMagick (Apache 2): used as a fallback assembler for alpha GIFs
        ...
        <ul style="margin: 0; padding-left: 18px;">
          ...
          <li><b><a href="https://gif.ski">gifski</a></b> by Kornel Lesiński —
              the high-quality GIF encoder used in the non-alpha pipeline.
              Licensed under AGPLv3.</li>
          <li><b><a href="https://imagemagick.org">ImageMagick</a></b> —
              used as a fallback assembler for transparent GIFs.
        ...
        <ul style="margin: 0; padding-left: 18px;">
          ...
          <li><b><a href="https://www.pyinstaller.org">PyInstaller</a></b> —
              produces the standalone Windows executable. GPL with bootloader exception.</li>
          <li><b><a href="https://upx.github.io">UPX</a></b> — final-stage
              compression of the .exe. GPLv2+ with linking exception.</li>
        </ul>
    ...
    def _refresh_trim_previews(self):
        ...
        out_sec = parse_trim_to_seconds(out_raw, default=None, fps=fps) if out_raw else None
    ...
    def _update_batch_trim_previews(self, video_path, in_sec, out_sec, dur, fps):
        ...
        if out_sec is None:
            out_sec = dur
        pix_in = extract_frame_at(video_path, in_sec, fps=fps, tw=120, th=68)
        last_sec = max(0.0, out_sec - (1.0 / fps if fps and fps > 0 else 0.04))
        pix_out = extract_frame_at(video_path, last_sec, fps=fps, tw=120, th=68)
        if pix_in:
            self._batch_thumb_in.setPixmap(pix_in)
        else:
            self._batch_thumb_in.clear(); self._batch_thumb_in.setText("—")
        if pix_out:
            self._batch_thumb_out.setPixmap(pix_out)
        else:
            self._batch_thumb_out.clear(); self._batch_thumb_out.setText("—")
